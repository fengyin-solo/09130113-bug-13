...
async def delete_project(
    ...
    # 清理关联地震数据在对象存储中的文件（与删除地震数据的逻辑保持一致）
    seismic_records = db.query(models.SeismicData).filter(
        models.SeismicData.project_id == project_id
    ).all()
    for seismic in seismic_records:
        if seismic.file_path and storage_service.file_exists(seismic.file_path):
            storage_service.delete_file(seismic.file_path)

        db.query(models.DataProcessingTask).filter(
            models.DataProcessingTask.seismic_data_id == seismic.id
