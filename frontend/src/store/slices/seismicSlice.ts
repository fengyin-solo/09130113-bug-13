...
const seismicSlice = createSlice({
  ...
  reducers: {
    ...
    clearSeismicForProject: (state, action: PayloadAction<number>) => {
      const projectId = action.payload;
      state.seismicList = state.seismicList.filter((s) => s.project_id !== projectId);
      if (state.currentSeismic?.project_id === projectId) {
